...
RType::Network::BoostNetwork::BoostNetwork(uint32_t port) : _logger(
    std::make_shared<Common::Log::Log>("network", "network.log", Common::Log::g_AllLogLevel, std::ios::trunc)), _threadPool(1),
    _pending_client(std::make_shared<ThreadSafeQueue < AClient < Socket::boost_socket_udp_t, Socket::boost_socket_tcp_t> * >> ()),
...
{
    this->_router.set_acceptor(*this->_router.get_io_service(), boost_asio_tcp::endpoint(boost_asio_tcp::v4(), port));
    this->_router.set_signal_set(*this->_router.get_io_service(), SIGINT);
}

void RType::Network::BoostNetwork::run()
{
    this->_logger->Info("Network listening on port ", this->_router.get_io_acceptor()->local_endpoint().port(), " for a new client...");
...
void RType::Network::BoostNetwork::stop()
{
    ...
    std::for_each(this->_clients.begin(), this->_clients.end(), [](const client_shared_ptr &client)
    {
...
void RType::Network::BoostNetwork::wait_for_client()
{
    ...
    auto client = std::make_shared<BoostClient>(*this->_router.get_io_service(), this->_logger, this->_worker_pending_client.share_cv_from_this(),
        this->_pending_client->shared_from_this());
    this->_clients.emplace_back(client);
    this->_router.get_io_acceptor()->async_accept(*client->get_tcpsocket()->get_socket(), [&, client](const boost::system::error_code &error)
    {
        if (error)
        {
        ...
        this->_logger->Debug("I will recreate a client for the next connection");
        this->wait_for_client();
    });
...
void RType::Network::BoostNetwork::pre_run()
{
    ...
    this->_router.get_signal_set()->async_wait([self = this->shared_from_this()](const boost::system::error_code &error, int /*signal*/)
    {
        if (error)
